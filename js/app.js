...
function createTaskTemplate(task, index) {
  ...
  const dateText = task.is_done
    ? "Done"
    : `Should be done: ${date.toLocaleString("en-EU", {
        year: "2-digit",
        month: "numeric",
        day: "2-digit",
      })}`;
...
function renderAllTasks(tasksList) {
  const fullTemplate = tasksList.reduce(
    (acc, task, index) => `${acc} ${createTaskTemplate(task, index)}`,
    ""
  );
...
function searchTasks(value) {
  ...
  if (value.trim() === "") {
    alert("Please enter symbols for searching");
  }
}
